...
class C_First_Order(object):
	...
	def define_deformed_subset(self, q, Xp, Yp):
		...
		self.i = np.arange(-half_subset, half_subset + 1, dtype=int)
		self.j = np.arange(-half_subset, half_subset + 1, dtype=int)

		self.I_matrix, self.J_matrix = np.meshgrid(self.i, self.j)
